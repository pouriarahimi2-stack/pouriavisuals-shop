...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 inset-x-0 z-40 bg-[var(--bg-primary)]/85 backdrop-blur-xl border-b border-[var(--card-border)] transition-colors">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
        
        {/* سمت راست: لوگوی تنظیم‌شده در ادمین و لینک‌های منو */}
        <div className="flex items-center gap-6">
          ...
          >
            {logoUrl ? (
              <img src={logoUrl} alt={siteName} className="h-9 w-auto max-w-[120px] object-contain rounded-lg" />
            ) : (
              <AnimatedLogo />
            )}
