...
export default function ProductsCatalogPage() {
  ...
  const filteredProducts = useMemo(() => {
    return products
      .filter((p) => {
        ...
        const currentPrice = Number(p.discount_price || p.discountPrice || p.price || 0);
        const matchesPrice = maxPrice === "" || currentPrice <= Number(maxPrice);
  ...
  }, [products, searchTerm, selectedCategory, onlyAvailable, maxPrice, sortBy]);
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-primary)] text-[var(--text-primary)] p-4 sm:p-6 lg:p-12 dir-rtl font-sans select-none">
      <div className="max-w-7xl mx-auto flex items-center gap-2 text-xs text-[var(--text-secondary)] mb-6 font-bold">
        ...
        <span>/</span>
        <span className="text-[var(--text-primary)]">کاتالوگ کالاها</span>
      </div>
        ...
        <div className="p-4 rounded-3xl bg-[var(--modal-bg)] border border-[var(--card-border)] shadow-sm space-y-3 text-xs">
          <div className="flex flex-wrap items-center gap-2">
            <button
              ...
              className={`px-3 py-1.5 rounded-xl font-bold transition ${selectedCategory === "all" ? "bg-[var(--accent-blue)] text-white" : "bg-[var(--input-bg)] text-[var(--text-secondary)]"}`}
            >
            ...
            {categories.map((cat) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-xl font-bold transition ${selectedCategory === cat ? "bg-[var(--accent-blue)] text-white" : "bg-[var(--input-bg)] text-[var(--text-secondary)]"}`}
              >
                {cat}
              </button>
            ))}
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 pt-2 border-t border-[var(--card-border)]">
            <div className="flex items-center gap-3">
              <button
                type="button"
                onClick={() => setOnlyAvailable(!onlyAvailable)}
                className={`px-3.5 py-2 rounded-xl text-xs font-bold flex items-center gap-1.5 transition cursor-pointer ${
                  onlyAvailable ? "bg-emerald-500 text-white shadow" : "bg-[var(--input-bg)] text-[var(--text-secondary)] border border-[var(--card-border)]"
                }`}
              >
              ...
              <div className="flex items-center gap-1.5">
                <span className="text-[var(--text-secondary)] font-bold">سقف قیمت (تومان):</span>
                <input
                  type="number"
                  placeholder="بدون محدودیت"
                  value={maxPrice}
                  onChange={(e) => setMaxPrice(e.target.value ? Number(e.target.value) : "")}
                  className="w-32 p-1.5 rounded-xl bg-[var(--input-bg)] border border-[var(--card-border)] font-mono text-center outline-none"
                />
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                className="bg-[var(--input-bg)] border border-[var(--card-border)] rounded-xl px-3 py-1.5 text-xs font-bold cursor-pointer outline-none"
              >
