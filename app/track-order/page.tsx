...
function TrackOrderContent() {
  ...
  const getStatusBadge = (status: OrderData["status"]) => {
    switch (status) {
      ...
      case "delivered":
        return <span className="px-3 py-1 rounded-full bg-emerald-500/15 text-emerald-500 border border-emerald-500/30 font-bold text-xs">✅ تحویل مرسوله به مشتری</span>;
      case "cancelled":
        return <span className="px-3 py-1 rounded-full bg-rose-500/15 text-rose-500 border border-rose-500/30 font-bold text-xs">❌ سفارش لغو شده</span>;
      default:
        return <span className="px-3 py-1 rounded-full bg-slate-500/15 text-slate-400 border border-slate-500/30 font-bold text-xs">⏳ در انتظار تأیید</span>;
    }
  };
