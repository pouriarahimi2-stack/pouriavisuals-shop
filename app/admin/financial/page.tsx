...
import {
  BarChart3, Package, Users, TrendingUp, Search, RefreshCw,
  Save, Download, AlertTriangle, CheckCircle2, Plus, Minus, ArrowUp, ArrowDown
} from "lucide-react";

type Tab = "orders" | "inventory" | "monthly" | "crm";

const TABS: { id: Tab; label: string; icon: React.ReactNode }[] = [
  { id: "orders",    label: "سفارشات و بارنامه", icon: <Package size={15} /> },
  { id: "inventory", label: "انبار و موجودی",    icon: <BarChart3 size={15} /> },
  { id: "monthly",   label: "گزارش مالی ماهانه", icon: <TrendingUp size={15} /> },
  { id: "crm",       label: "مشتریان CRM",       icon: <Users size={15} /> },
];
...
const STATUS_MAP: Record<string, { label: string; color: string }> = {
  ...
  shipped:   { label: "ارسال‌شده",  color: "text-blue-400    bg-blue-400/10    border-blue-400/20"    },
  delivered: { label: "تحویل‌شده",  color: "text-green-500   bg-green-500/10   border-green-500/20"   },
  cancelled: { label: "لغو‌شده",   color: "text-rose-400    bg-rose-400/10    border-rose-400/20"    },
};

function toFaMonth(m: string) {
  const months = ["فروردین","اردیبهشت","خرداد","تیر","مرداد","شهریور","مهر","آبان","آذر","دی","بهمن","اسفند"];
  const [y, mon] = m.split("-");
  return months[Number(mon) - 1] + " " + new Date(Number(y), 0).toLocaleDateString("fa-IR", { year: "numeric" }).split("/")[0];
}
...
function OrdersTab() {
  ...
  const updateStatus = async (id: string, s: string) => {
    await fetch("/api/admin/orders", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, payment_status: s }) });
  ...
  const saveTracking = async (id: string) => {
    ...
    const r    = await fetch("/api/admin/orders", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, tracking_code: code, payment_status: code ? "shipped" : undefined }) });
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="جستجوی نام، موبایل، شماره سفارش..." className="w-full py-2.5 pr-9 pl-3 rounded-2xl bg-[var(--input-bg)] border border-[var(--card-border)] text-xs font-bold outline-none" />
        </div>
        <select value={filter} onChange={e=>setFilter(e.target.value)} className="p-2.5 rounded-2xl bg-[var(--input-bg)] border border-[var(--card-border)] text-xs font-bold outline-none cursor-pointer">
          <option value="all">همه وضعیت‌ها</option>
          {Object.entries(STATUS_MAP).map(([k,v]) => <option key={k} value={k}>{v.label}</option>)}
        </select>
        <button onClick={load} className="p-2.5 rounded-2xl bg-[var(--input-bg)] border border-[var(--card-border)] hover:bg-[var(--card-hover)] transition"><RefreshCw size={15} /></button>
      </div>

      <div className="text-xs text-[var(--text-secondary)] font-bold">{orders.length} سفارش ثبت‌شده · نمایش {filtered.length} مورد</div>

      {loading ? <div className="py-16 text-center text-xs text-slate-400">در حال بارگذاری...</div> :
       filtered.length === 0 ? <div className="py-16 text-center text-xs text-slate-400">سفارشی یافت نشد</div> :
      <div className="space-y-3">
        {filtered.map(ord => {
          const s = STATUS_MAP[ord.payment_status] || { label: ord.payment_status, color: "text-slate-400 bg-slate-400/10 border-slate-400/20" };
          return (
            <div key={ord.id} className="bg-[var(--card-bg)] border border-[var(--card-border)] rounded-3xl p-5 space-y-3">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div className="flex flex-wrap items-center gap-2 text-xs">
                  ...
                  {ord.phone_verified && <span className="px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 text-[10px] font-bold">✓ OTP</span>}
                </div>
                <div className="flex items-center gap-2">
                  <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold border ${s.color}`}>{s.label}</span>
                  <select value={ord.payment_status||"pending"} onChange={e=>updateStatus(ord.id,e.target.value)} className="bg-[var(--input-bg)] border border-[var(--card-border)] text-[11px] font-bold px-2 py-1 rounded-xl outline-none cursor-pointer">
                    {Object.entries(STATUS_MAP).map(([k,v]) => <option key={k} value={k}>{v.label}</option>)}
                  </select>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-[11px]">
                <div><span className="text-slate-400">آدرس:</span> <span className="font-medium">{ord.shipping_address}</span></div>
                <div><span className="text-slate-400">مبلغ:</span> <span className="font-mono font-black text-emerald-400">{Number(ord.total_price||0).toLocaleString("fa-IR")} ت</span></div>
              ...
              {Array.isArray(ord.items) && ord.items.length > 0 && (
                <div className="flex flex-wrap gap-1.5">
                  {ord.items.map((it:any,i:number) => (
                    <span key={i} className="px-2.5 py-1 rounded-xl bg-[var(--input-bg)] border border-[var(--card-border)] text-[10px] font-bold">{it.title} ×{it.quantity}</span>
                  ))}
                </div>
              )}

              <div className="flex items-center gap-2 pt-2 border-t border-[var(--card-border)]">
                <input value={tracking[ord.id]||""} onChange={e=>setTracking(p=>({...p,[ord.id]:e.target.value}))} placeholder="کد رهگیری پست (۲۴ رقم)..." className="flex-1 p-2 rounded-xl bg-[var(--input-bg)] border border-[var(--card-border)] font-mono text-[11px] outline-none" />
                <button onClick={()=>saveTracking(ord.id)} disabled={saving[ord.id]} className="px-3 py-2 rounded-xl bg-[var(--accent-blue)] text-white text-[11px] font-bold flex items-center gap-1 disabled:opacity-50 cursor-pointer">
...
function InventoryTab() {
  ...
  const handleRestock = async (p: any) => {
    ...
    const ok = window.confirm(`آیا مطمئنید که می‌خواهید ${qty} عدد به موجودی «${p.title}» اضافه کنید؟
موجودی فعلی: ${p.stock} عدد → جدید: ${Number(p.stock)+qty} عدد`);
    if (!ok) return;
    setSaving(s => ({...s,[p.id]:true}));
    const r = await fetch("/api/admin/financial", { method: "PATCH", headers:{"Content-Type":"application/json"}, body: JSON.stringify({ productId: p.id, addStock: qty }) });
    const d = await r.json();
  ...
  return (
    <div className="space-y-4">
      ...
      {stats && <div className="text-xs text-[var(--text-secondary)]">ارزش کل انبار: <span className="font-mono font-black text-emerald-400">{Number(stats.totalInventoryValue).toLocaleString("fa-IR")} تومان</span></div>}

      <div className="relative">
        <Search size={14} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400" />
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="جستجوی نام یا دسته محصول..." className="w-full py-2.5 pr-9 pl-3 rounded-2xl bg-[var(--input-bg)] border border-[var(--card-border)] text-xs font-bold outline-none" />
      </div>

      {loading ? <div className="py-12 text-center text-xs text-slate-400">در حال بارگذاری انبار...</div> :
      <div className="overflow-x-auto rounded-3xl border border-[var(--card-border)]">
        <table className="w-full text-xs">
          ...
          <tbody className="divide-y divide-[var(--card-border)]">
            {filtered.map(p => (
              <tr key={p.id} className={`hover:bg-[var(--card-hover)] transition ${Number(p.stock)===0?"bg-rose-500/5":Number(p.stock)<=3?"bg-amber-500/5":""}`}>
                ...
                <td className="py-2.5 px-4">
                  <span className={`font-black font-mono ${Number(p.stock)===0?"text-rose-400":Number(p.stock)<=3?"text-amber-400":"text-emerald-400"}`}>
                    {p.stock}
                    {Number(p.stock)<=3 && Number(p.stock)>0 && <AlertTriangle size={12} className="inline mr-1 text-amber-400" />}
                ...
                <td className="py-2.5 px-4">
                  <div className="flex items-center gap-1.5">
                    <input type="number" min={1} value={addQty[p.id]||""} onChange={e=>setAddQty(a=>({...a,[p.id]:e.target.value}))}
                      placeholder="تعداد" className="w-16 p-1.5 rounded-xl bg-[var(--input-bg)] border border-[var(--card-border)] font-mono text-[11px] text-center outline-none" />
...
function MonthlyTab() {
  ...
  const exportCSV = () => {
    ...
    a.download = `axon-report-${month}.csv`; a.click();
  };
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <button onClick={exportCSV} className="px-4 py-2.5 rounded-2xl bg-[var(--input-bg)] border border-[var(--card-border)] text-xs font-bold flex items-center gap-2 hover:bg-[var(--card-hover)] transition cursor-pointer">
          <Download size={14} /> دریافت CSV
        </button>
        {data && <span className="text-xs text-[var(--text-secondary)]">{toFaMonth(month)}</span>}
      </div>

      {loading ? <div className="py-12 text-center text-xs text-slate-400">در حال محاسبه گزارش...</div> :
       data && <>
        {/* KPI cards */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
          {[
            ...
            <div className="flex items-end gap-1 h-28 overflow-x-auto pb-4">
              {data.dailyRevenue.map((d:any) => (
                <div key={d.date} className="flex flex-col items-center gap-1 min-w-[28px] flex-1">
                  <div title={d.date+": "+Number(d.revenue).toLocaleString("fa-IR")+" ت"}
                    style={{ height: `${Math.max(4, (d.revenue/maxRevenue)*90)}px` }}
                    className="w-full rounded-t-lg bg-[var(--accent-blue)] opacity-80 hover:opacity-100 transition cursor-pointer" />
                  <span className="text-[8px] text-slate-400 rotate-45 origin-top-left">{d.date.slice(8)}</span>
                </div>
              ))}
            </div>
          </div>
...
function CRMTab() {
  ...
  return (
    <div className="space-y-4">
      <div className="relative">
        <Search size={14} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400" />
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="جستجوی نام یا شماره..." className="w-full py-2.5 pr-9 pl-3 rounded-2xl bg-[var(--input-bg)] border border-[var(--card-border)] text-xs font-bold outline-none" />
      </div>

      <div className="text-xs text-[var(--text-secondary)]">{customers.length} مشتری · {customers.filter(c=>c.isVip).length} VIP</div>

      {loading ? <div className="py-12 text-center text-xs text-slate-400">در حال بارگذاری...</div> :
      <div className="overflow-x-auto rounded-3xl border border-[var(--card-border)]">
        <table className="w-full text-xs">
          <thead>
            <tr className="bg-[var(--card-bg)] border-b border-[var(--card-border)] text-[var(--text-secondary)] font-bold">
              ...
              <th className="py-3 px-4 text-right">آخرین خرید</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-[var(--card-border)]">
            {filtered.map((c,i) => (
              <tr key={i} className="hover:bg-[var(--card-hover)] transition">
                <td className="py-2.5 px-4">
                  <div className="flex items-center gap-2">
                    <div className="w-8 h-8 rounded-xl bg-[var(--accent-blue)]/20 text-[var(--accent-blue)] flex items-center justify-center font-black text-sm">{(c.name||"?")[0]}</div>
                    <div>
                ...
                <td className="py-2.5 px-4 font-mono text-slate-400">{c.phone}</td>
                <td className="py-2.5 px-4">
                ...
                <td className="py-2.5 px-4 text-slate-400">{new Date(c.lastOrderAt).toLocaleDateString("fa-IR")}</td>
              </tr>
            ))}
          </tbody>
        </table>
            ...
            <button key={t.id} onClick={() => setTab(t.id)}
              className={`px-4 py-2 rounded-2xl text-xs font-bold flex items-center gap-1.5 transition cursor-pointer ${
                tab === t.id
                  ...
                  : "bg-[var(--input-bg)] border border-[var(--card-border)] hover:bg-[var(--card-hover)]"
              }`}>
              {t.icon}{t.label}
            </button>
          ))}
