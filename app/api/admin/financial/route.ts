...
export async function GET(req: NextRequest) {
  ...
  try {
    if (type === "monthly") {
      ...
      const all       = orders || [];
      const paid      = all.filter((o: any) => ["paid","delivered","shipped"].includes(o.payment_status || o.status || ""));
      const cancelled = all.filter((o: any) => (o.payment_status || o.status) === "cancelled");
      const totalRevenue = paid.reduce((s: number, o: any) => s + Number(o.final_amount || o.total_amount || o.total_price || 0), 0);
      ...
      return NextResponse.json({ success: true, month,
        stats: { totalOrders: all.length, paidOrders: paid.length, cancelledOrders: cancelled.length,
        ...
        dailyRevenue, topProducts, orders: all });
    }
    if (type === "inventory") {
      ...
      const all        = products || [];
      const totalValue = all.reduce((s: number, p: any) => s + Number(p.price || 0) * Number(p.stock || 0), 0);
      return NextResponse.json({ success: true, products: all,
        stats: { totalProducts: all.length,
          inStock: all.filter((p: any) => Number(p.stock) > 0).length,
          outOfStock: all.filter((p: any) => Number(p.stock) === 0).length,
          lowStock: all.filter((p: any) => Number(p.stock) > 0 && Number(p.stock) <= 3).length,
    ...
    if (type === "customers") {
      ...
      (orders || []).forEach((o: any) => {
        ...
        if (!customerMap[phone]) customerMap[phone] = {
          phone, name: o.customer_name || "نامشخص",
          totalOrders: 0, paidOrders: 0, totalSpent: 0,
          lastOrderAt: o.created_at, firstOrderAt: o.created_at };
        const c = customerMap[phone];
        c.totalOrders++;
        if (["paid","delivered","shipped"].includes(o.payment_status || o.status || "")) {
          c.paidOrders++; c.totalSpent += Number(o.final_amount || o.total_amount || o.total_price || 0);
        }
...
export async function PATCH(req: NextRequest) {
  ...
  try {
    ...
    if (!productId) return NextResponse.json({ success: false, message: "productId الزامی" }, { status: 400 });
    ...
    const { error } = await supabaseAdmin.from("products")
      .update({ stock: Math.max(0, finalStock), is_available: finalStock > 0 }).eq("id", productId);
    if (error) throw error;
    return NextResponse.json({ success: true, newStock: finalStock,
      message: "✓ موجودی " + (current?.title || "") + " به " + finalStock + " عدد بروزرسانی شد." });
  } catch (err: any) {
    return NextResponse.json({ success: false, message: err.message }, { status: 500 });
  }
}