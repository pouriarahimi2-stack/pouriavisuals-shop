    ...
    <div className="min-h-screen py-10 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto font-sans select-none text-[var(--text-primary)] space-y-10" dir="rtl">
      ...
      <div className="p-8 sm:p-12 rounded-[2.5rem] bg-gradient-to-r from-blue-950/40 via-indigo-950/30 to-[var(--modal-bg)] border border-[var(--card-border)] shadow-2xl flex flex-col md:flex-row items-start md:items-center justify-between gap-6 backdrop-blur-3xl">
        ...
        <button
          ...
          disabled={syncing}
          className="px-6 py-3.5 rounded-2xl bg-[var(--accent-blue)] text-white font-black text-xs hover:opacity-90 transition shadow-xl shadow-blue-500/25 cursor-pointer disabled:opacity-50 flex items-center gap-2 shrink-0"
        >
      ...
      {loading && news.length === 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div key={i} className="h-80 rounded-[2.5rem] bg-[var(--input-bg)] animate-pulse border border-[var(--card-border)]" />
          ))}
        </div>
      ) : filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map((item) => (
            <article
              ...
              className="rounded-[2.5rem] bg-[var(--modal-bg)] border border-[var(--card-border)] overflow-hidden shadow-xl hover:border-[var(--accent-blue)] hover:shadow-2xl transition duration-300 flex flex-col justify-between group cursor-pointer"
            >
          ...
          >
            <header className="p-4 sm:p-6 border-b border-[var(--card-border)] flex items-center justify-between bg-[var(--input-bg)]">
              ...
              <button
                ...
                className="w-10 h-10 rounded-2xl bg-[var(--modal-bg)] hover:bg-rose-500 hover:text-white border border-[var(--card-border)] flex items-center justify-center text-sm font-black transition cursor-pointer shadow-md"
                title="بستن پنجره"
              >
                ✕
              </button>
            ...
            <div className="flex-1 overflow-y-auto p-6 sm:p-10 space-y-6 text-xs sm:text-sm">
              ...
              <div className="w-full h-64 sm:h-96 rounded-3xl overflow-hidden bg-[var(--input-bg)] border border-[var(--card-border)] shadow-inner">
                <img
              ...
              <div
                ...
                className="prose max-w-none text-xs sm:text-sm leading-loose text-[var(--text-primary)] font-medium space-y-4 text-justify"
              />
              ...
              {activeModalNews.tags && activeModalNews.tags.length > 0 && (
                <div className="flex flex-wrap gap-2 pt-4 border-t border-[var(--card-border)]">
                  {activeModalNews.tags.map((t) => (
                    <span
                      key={t}
                      className="px-3 py-1 rounded-xl bg-[var(--input-bg)] border border-[var(--card-border)] text-[10px] font-bold text-[var(--text-secondary)]"
                    >
                      #{t}
                    </span>
                  ))}
            ...
            <footer className="p-4 sm:p-6 border-t border-[var(--card-border)] flex flex-wrap items-center justify-between gap-4 bg-[var(--input-bg)]">
              <Link
                href="/#products"
                onClick={closeNewsModal}
                className="px-6 py-3 rounded-2xl bg-[var(--accent-blue)] text-white font-black text-xs hover:opacity-90 transition shadow-lg flex items-center gap-2"
              >
                <span>🛍️</span>
                <span>مشاهده تجهیزات و مانیتورهای مرتبط در فروشگاه</span>
              </Link>

              <button
                onClick={closeNewsModal}
                className="px-6 py-3 rounded-2xl bg-[var(--modal-bg)] border border-[var(--card-border)] text-xs font-bold hover:border-[var(--accent-blue)] transition cursor-pointer"
              >
                بستن پنجره ✕
              </button>
            </footer>
