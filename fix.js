...
console.log('🎬 [AXON ARCHITECT] در حال افزودن دکمه نمایش/مخفی‌سازی رمز عبور (Show/Hide Password Eye) به ماژول مدیریت حساب‌های ادمین...');

const files = {
  // ۱. ماژول مدیریت حساب‌ها مجهز به دکمه چشم نمایش/پنهان‌سازی رمز عبور
...
for (const [filePath, content] of Object.entries(files)) {
  ...
  console.log(`✅ [PASSWORD EYE TOGGLE READY] فایل با موفقیت به‌روزرسانی شد: ${filePath}`);
}

console.log('📦 در حال Push به گیت‌هاب و دیپلوی خودکار روی Vercel...');
try {
  execSync('git add . && git commit -m "feat: add show/hide eye toggle button for password input in AdminAccountsManager" && git push origin main', { stdio: 'inherit' });
