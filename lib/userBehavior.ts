 ...
 * پیاده‌سازی هوشمند جهت افزایش ماندگاری و رنکینگ محصولات
 */
...
export const userBehavior = {
  ...
  trackNewsRead(slug: string, category: string) {
    ...
    const catKey = (category || "gadgets").toLowerCase().trim();
    p.categories[catKey] = (p.categories[catKey] || 0) + 3;
    this.saveProfile(p);
  },
  ...
  trackProductView(productId: string, category: string) {
    ...
    p.categories[catKey] = (p.categories[catKey] || 0) + 5;
    this.saveProfile(p);
  },
